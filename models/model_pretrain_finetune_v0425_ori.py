...
from modules.utils import (GlobalEmbedding, LocalEmbedding, init_weights, get_extended_attention_mask,
                           VisualProjectionHead, TextProjectionHead, ScaledDotProductAttention)
...
class FineTune(nn.Module):
    def __init__(self, args: dict, tokenizer: object, data_name: str):
        ...
        self.visual_head = VisualProjectionHead(visual_dim, output_dim=args['output_dim'], hidden_dim=args['output_dim'])
        self.text_head = TextProjectionHead(text_dim, output_dim=args['output_dim'], hidden_dim=args['output_dim'])
    ...
    def multiview_fusion(self, global_image_embed, local_image_embed, patient_ids, batch_size):
        ...
        image_embed = self.layer_norm(image_embed)
        new_image_embed = []
        for i in range(batch_size):
            ...
            # add & normalize
            # add & normalize
            # cur_image_embed = self.layer_norm(cur_image_embed + image_embed[i])
            cur_image_embed = self.layer_norm(cur_image_embed + image_embed[i])
...
class Pretrain(nn.Module):
    def __init__(self, args: dict, tokenizer: object, data_name: str):
        ...
        # define the local embedding and global embedding for uni-modal
        text_dim = self.text_encoder.encoder.config.hidden_size
        self.visual_head = VisualProjectionHead(visual_dim, output_dim=args['output_dim'],
