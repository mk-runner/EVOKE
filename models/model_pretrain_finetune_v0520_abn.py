...
class FineTune(nn.Module):
    def __init__(self, args: dict, tokenizer: object, data_name: str):
        ...
        if args['text_decoder'] == 'r2gen':
            ...
            if len(args['generator_checkpoint']) != 0:

                model_dict = self.text_decoder.state_dict()
                pretrain_dict = torch.load(args['generator_checkpoint'])
                valid_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict and v.shape == model_dict[k].shape}
                model_dict.update(valid_dict)
                self.text_decoder.load_state_dict(model_dict)
                print('finish loading pre-trained model of generator')
