...
class FineTune(nn.Module):
    ...
    def multiview_fusion(self, global_image_embed, local_image_embed, patient_ids, batch_size):
        ...
        for i in range(batch_size):
            ...
            cur_image_embed_2 = self.multiview_cross_attention(multiview_image_embed, image_embed[i].detach(),
                                                               image_embed[i].detach())
            # add & normalize
            cur_image_embed = self.layer_norm_2(cur_image_embed + image_embed[i])
            cur_image_embed_2 = self.layer_norm_2(cur_image_embed_2 + multiview_image_embed)
            cur_image_embed = torch.cat([cur_image_embed, cur_image_embed_2])
