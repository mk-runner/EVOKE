...
class PretrainBaseDatasetHasMultiview(Dataset):  # finetune and inference phase
    def __init__(self, args, tokenizer, split, transform=None):
        ...
        if args['align_type'] == 'keywords':
            for i in range(len(ann)):
                if len(ann[i]['core_findings']) == 0 or len(ann[i]['image_path']) < 2:
...
class IuxrayPretrainDatasetHasMultiview(PretrainBaseDatasetHasMultiview):  # finetune and inference phase
...
class MimiccxrPretrainDatasetHasMultiview(PretrainBaseDatasetHasMultiview):
    def __getitem__(self, idx):
        example = self.examples[idx]
        image_id = example['id']

        sentence_idx, radgraph_ids = [], []  # sentence_index and factual serialization's input_ids
        for sen_id, item_fs in enumerate(example['radgraph']):
            item_fs = item_fs + ' [SEP]'
            if sen_id == 0:
                item_fs = '[CLS] ' + item_fs
            item_ids = self.tokenizer.encode(item_fs).ids
            radgraph_ids.extend(item_ids)
            sentence_idx.extend([sen_id] * len(item_ids))
        radgraph_ids = radgraph_ids[:self.max_seq_length]
        sentence_idx = sentence_idx[1:self.max_seq_length]
        radgraph_len = len(radgraph_ids)
        radgraph_masks = [1] * radgraph_len
        ...
        for image_path in example['image_path']:
            ...
            images.append(image)
        sample = (image_id, images, radgraph_ids, radgraph_masks, radgraph_len, sentence_idx)
        return sample


class PretrainBaseDatasetNotMultiview(Dataset):  # finetune and inference phase
    def __init__(self, args, tokenizer, split, transform=None):
        self.transform = transform
        self.tokenizer = tokenizer
        self.image_dir = args['image_dir']
        ...
        if args['align_type'] == 'keywords':
            for i in range(len(ann)):
                if len(ann[i]['core_findings']) == 0:
                    continue
                # if is_multiview_learning, only pass one image. otherwise all image is passing
                # note that each image_path include one image.
                if args['is_multiview_learning'] and len(ann[i]['image_path']) >= 2:
                    continue
                core_findings = ann[i]['core_findings']
                if args['tokenizer_type'] == 'uncased':
                    core_findings = list(map(lambda x: str(x).lower(), core_findings))
...
class IuxrayPretrainDatasetNotMultiview(PretrainBaseDatasetNotMultiview):  # finetune and inference phase
    def __getitem__(self, idx):
        example = self.examples[idx]
        image_id = example['id']
        image_path = example['image_path']
...
class MimiccxrPretrainDatasetNotMultiview(PretrainBaseDatasetNotMultiview):
    def __getitem__(self, idx):
        example = self.examples[idx]
        image_id = example['id']
        ...
        if self.transform is not None:
            image = self.transform(image)
        sample = (image_id, image, radgraph_ids, radgraph_masks, radgraph_len, sentence_idx)
        return sample
...
class MimiccxrFinetuneDatasetHasIndication(FinetuneBaseDatasetHasIndication):  # finetune and inference phase
    def __getitem__(self, idx):
        ...
        sample = (image_id, images, report_ids, report_masks, report_len, inc_ids, inc_masks, inc_len)
        return sample
...
class MimiccxrFinetuneDatasetNotIndication(FinetuneBaseDatasetNotIndication):  # finetune and inference phase
    def __getitem__(self, idx):
        ...
        sample = (image_id, images, report_ids, report_masks, report_len)
        return sample
