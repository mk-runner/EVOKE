...
def compute_chexbert_details_scores(gts, res, args):
    f1chexbert = F1CheXbert(chexbert_checkpoint=args['chexbert_checkpoint'],
                            model_checkpoint=args['chexbert_model_checkpoint'],
                            tokenizer_checkpoint=args['chexbert_tokenizer_checkpoint'])
    accuracy, accuracy_per_sample, chexbert_all, chexbert_5 = f1chexbert(hyps=res, refs=gts)
    # default is chexbert_5_micro_f1
    # micro: each sample has the same weight; macro: each class has the same weight
...
def compute_b4_rg(gts, res, args):
    def compute_b4(gts, res):
        gts = {i: [gt] for i, gt in enumerate(gts)}
        res = {i: [re] for i, re in enumerate(res)}
        scorers = [
            (Bleu(4), ["BLEU_1", "c", "BLEU_3", "BLEU_4"]),
        ]
        eval_res = {}
        # Compute score for each metric
        for scorer, method in scorers:
    ...
    eval_res = compute_b4(gts, res)
    # compute ce scores
    f1radgraph_partial = F1RadGraph(reward_level='partial', model_path=args['radgraph_checkpoint'])
    partial_mean_reward, reward_list, hypothesis_ann_lists, reference_ann_lists = f1radgraph_partial(hyps=res, refs=gts)
    return {'B4 & RG': f'{round(eval_res["BLEU_4"], 4)} & {round(partial_mean_reward, 3)}'}


# def temp_main():
#     gts = ['as compared to the previous radiograph , there is complete resolution of the pre - existing pleural effusions . unchanged moderate cardiomegaly without evidence of pulmonary edema . small basal parenchymal scars but no evidence of recent pneumonia . moderate tortuosity of the thoracic aorta . calcified bronchial walls .',
#            'pa and lateral views of the chest provided . there is no focal consolidation , effusion , or pneumothorax . the cardiomediastinal silhouette is stable and top - normal in size . imaged osseous structures are intact . no free air below the right hemidiaphragm is seen .'
#            ]
#     res = [
#         'as compared to the previous radiograph , there is no relevant change . moderate cardiomegaly with tortuosity of the thoracic aorta but no evidence of pulmonary edema or pneumonia . no pleural effusions . no lung nodules or masses .',
...
#     ]
#     config = '/home/miao/data/Code/Second/config/finetune_config.yaml'
#     args = yaml.load(open(config), Loader=yaml.FullLoader)
#     # ce_metrics = compute_ce_scores(gts, res, args)
#     ce_metrics = compute_chexbert_details_scores(gts, res, args)
#     print(ce_metrics)
#
# temp_main()
