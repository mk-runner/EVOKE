...
class BaseTrainer(object):
    ...
    def _load_checkpoint(self, load_path):
        ...
        # temp = [k for k in checkpoint['state_dict'] if 'text_head' in k]
        # temp1 = [k for k in self.model.state_dict() if 'text_head' in k]
...
class PTrainer(BaseTrainer):
    def __init__(self, model, metric_ftns, optimizer, args, lr_scheduler, train_loader, val_loader,
                 test_loader, logger, task, runner, is_save_checkpoint, **kwargs):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (_, images, radgraph_ids, radgraph_masks, patient_ids) in enumerate(self.train_loader):
            ...
            if batch_idx % 1000 == 0 or batch_idx + 1 == len(self.train_loader):
...
class PTrainerTemp(BaseTrainer):
    def __init__(self, model, metric_ftns, optimizer, args, lr_scheduler, train_loader, val_loader,
                 test_loader, logger, task, runner, is_save_checkpoint, **kwargs):
        super(PTrainerTemp, self).__init__(model, metric_ftns, optimizer, args, task, is_save_checkpoint, logger)
        self.lr_scheduler = lr_scheduler
        self.train_loader = train_loader
        self.val_loader = val_loader
        self.test_loader = test_loader
    ...
    def _train_epoch(self, epoch):
        ...
        if self.train_loader_inc is not None:
            for batch_idx, (_, images, input_ids, attention_masks, patient_ids, inc_ids, inc_masks) in enumerate(
                ...
                torch.nn.utils.clip_grad_value_(self.model.parameters(), 0.1)
                self.optimizer.step()
                if batch_idx % 1000 == 0 or batch_idx + 1 == len(self.train_loader_inc):
        ...
        with torch.no_grad():
            ...
            if self.val_ids is None:
                self.val_ids = val_images_ids
                cur_val_ret.to_csv(val_pred_path, index=False)

            else:
                assert self.val_ids == val_images_ids
                val_pred_df = pd.read_csv(val_pred_path)
                val_pred_df[f'pred_report_{epoch}'] = cur_val_ret[f'pred_report_{epoch}']
        ...
        with torch.no_grad():
            ...
            if self.test_ids is None:
                self.test_ids = test_images_ids
                cur_test_ret.to_csv(test_pred_path, index=False)

            else:
                assert self.test_ids == test_images_ids
                test_pred_df = pd.read_csv(test_pred_path)
                test_pred_df[f'pred_report_{epoch}'] = cur_test_ret[f'pred_report_{epoch}']
