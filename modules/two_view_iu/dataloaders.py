...
class PretrainLoader(DataLoader):
    ...
    def collate_fn(self, data):
        ...
        image_ids, batch_images_list, radgraph_ids, radgraph_masks, radgraph_lens = zip(*data)
        # radgraph
        ...
        for i, (ids, images_path) in enumerate(zip(image_ids, batch_images_list)):
            # select anchor image
...
class FinetuneLoaderHasIndication(DataLoader):
    ...
    def collate_fn(self, data):
        # note that images is tuple(list, list)
        image_ids, batch_images_list, report_ids, report_masks, report_lens, inc_ids, inc_masks, inc_lens = zip(*data)
...
class FinetuneLoaderNotIndication(DataLoader):
    ...
    def collate_fn(self, data):
        # note that images is tuple(list, list)
        image_ids, batch_images_list, report_ids, report_masks, report_lens = zip(*data)
